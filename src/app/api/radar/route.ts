...
/**
 * OSINT — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'OSINT/4.2', 'Accept': 'application/json' },
    });

    console.log('[OSINT] IODA response status:', res.status);
    ...
    console.error('[OSINT] IODA fetch error:', error);
